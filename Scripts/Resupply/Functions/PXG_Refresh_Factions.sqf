...
// Populate TreeView: Level 1 (Faction)
{
	...
	// Populate TreeView: Level 2 (Sub-Faction)
	{
		...
		_sfIndex = tvAdd [451501, [_fIndex], _sfFormattedName];
		...
		// Populate TreeView: Level 3 (Variants)
		{
			...
			
			{
				...
				_vIndex = tvAdd [451501, [_fIndex, _sfIndex], _variantDisplayName];
				tvSetData [451501, [_fIndex, _sfIndex, _vIndex], str _metadata];
				...
				if (_uniformClass != "" && {isClass (configFile >> "CfgWeapons" >> _uniformClass)}) then {
					...
					if (_iconPath != "") then {
						tvSetPicture [451501, [_fIndex, _sfIndex, _vIndex], _iconPath];
					};
				};
				// --- End Camo Icon ---
			} forEach _camos;
		...
		tvExpand [451501, [_fIndex, _sfIndex]];
