...
if (_slotType in ["optic", "muzzle", "acc", "bipod"]) then {
    private _compat = [_dummy, _slotType] call (compile preprocessFile "Scripts\Faction_Builder_Tool\Functions\Armory\FBT_GetCompatibleItems.sqf");
...
if (_slotType in ["medic", "ammo", "grenade", "misc"]) then {
    ...
    if (_slotType == "ammo") then {
        private _compMags = call (compile preprocessFile "Scripts\Faction_Builder_Tool\Functions\Armory\FBT_GetCompatibleMags.sqf");
        {
            private _idx = _ctrlList lbAdd (_x select 0);
            _ctrlList lbSetPicture [_idx, _x select 2];
            _ctrlList lbSetData [_idx, _x select 1];
