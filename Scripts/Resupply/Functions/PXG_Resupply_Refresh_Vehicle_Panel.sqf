...
if (_indexSpawn == -1) exitWith {
	{ (findDisplay IDD_RESUPPLY displayCtrl _x) ctrlShow false } forEach [IDC_PANEL_BG, IDC_PANEL_NAME, IDC_PANEL_PIC, IDC_PANEL_CAP, IDC_PANEL_LIST, IDC_PANEL_BTN, IDC_PANEL_HEADER];
};
...
// Show Panel
{ (_display displayCtrl _x) ctrlShow true } forEach [IDC_PANEL_BG, IDC_PANEL_NAME, IDC_PANEL_PIC, IDC_PANEL_CAP, IDC_PANEL_LIST, IDC_PANEL_BTN, IDC_PANEL_HEADER];
...
private _remainingSpace = (_maxSpace - _currentSpace);
private _color = if (_currentSpace < _maxSpace && _maxSpace > 0) then {"#00ff00"} else {"#ff0000"};

(_display displayCtrl IDC_PANEL_CAP) ctrlSetStructuredText parseText format [
	"Capacity:  <t color='%1'>%2</t> / %3", 
	_color, 
	_remainingSpace, 
	_maxSpace
];
